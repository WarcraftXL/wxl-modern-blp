...
namespace wxl::modern::blp
{
    ...
    {
        // Native character component textures top out around 256px on the long edge. Decoding a modern
        // 512/1024 top mip and then palettizing it makes the host visibly stall and produces oversized
        // palettized payloads for a compositor that was built around the smaller native shape.
        constexpr uint32_t kMaxComponentEdge = 256;

        RgbaImage img;
        if (!DecodeDxtMip(in, img, kMaxComponentEdge)) return false;
